...
public class BaseDao {
    String url = "jdbc:mysql://localhost:3306/mysql3?characterEncoding=UTF-8";
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                for (int i = 0; i < resultSetMetaData.getColumnCount(); i++) {
                    Field field = classType.getDeclaredField(resultSetMetaData.getColumnName(i + 1));
                    field.setAccessible(true);
                    String fileName = field.getName();
                    field.set(t, resultSet.getObject(fileName));
                }
                resultList.add(t);
            }
            connection.close();
